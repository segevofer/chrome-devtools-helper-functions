function findPathDFS(_obj, prop, limit = 10, exactOnly = false) {
  ...
  function isMatch(foundProp, fullPath) {
    if (typeof prop === 'string') {
      return foundProp === prop || (!exactOnly
          && foundProp.toLowerCase().indexOf(prop.toLowerCase()) != -1)
    }
  ...
  function _findPath(obj, currpath) {
    ...
    if (!obj || seen.has(obj) || /(.requirejs\.s|_reactInternalInstance)/.test(
        currpath) || currpath.length > 130) {
      return
    }
    seen.add(obj)
    for (var k in obj) {
