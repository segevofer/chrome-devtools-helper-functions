...
function isDomNode(node) {
  ...
  try {
    return typeof node.nodeType === "number" && typeof node.nodeName
        === "string"
  } catch (e) {
    return true
  }
...
function processNodeIfNeeded(prop, parent, key) {
  ...
  if (!(/(?:^requirejs$|_renderedComponent|_reactBoundContext|_reactInternalInstance|__key|__visited|__parent|__depth)/.test(
      key))) {
    ...
    if (shouldProcessNext(node, originalNode)) {
      ...
      if (isMatch(key, key, prop) && (!originalNode.__printed
          || !originalNode.__printed.has(key))) {
...
findPathBFS.get = keyOrRegex => {
  if (typeof keyOrRegex === 'string') {
    keyOrRegex = new RegExp(keyOrRegex)
  }
